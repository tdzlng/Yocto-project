...
static void led_jdg(void){
    if(strcmp(mdev.kmalloc_ptr, "on") == 0) {
        pr_info("LED is ON");
    // Set GPIO 27 high
    gpio_set_value(GPIO_27, HIGH);
    } else if (strcmp(mdev.kmalloc_ptr, "off") == 0) {
        pr_info("LED is OFF");
    // Set GPIO 27 low
    gpio_set_value(GPIO_27, LOW);
    } else {
...
static void gpio_jdg_init(void){
    ...
    int err = gpio_request(GPIO_27, "gpio-539");
    pr_info("gpio_request err: %d\n", err);
    gpio_direction_output(GPIO_27, LOW);

    my_delay(1000000);
    // Set GPIO27 to HIGH
    gpio_set_value(GPIO_27, HIGH);
    pr_info("GPIO set to HIGH1, status: %d!\n", gpio_get_value(GPIO_27));
...
static int __init gpiointerger_driver_init(void)
{
    ...
    pr_info("gpio interger kernel module\n");
    return 0;
...
static void __exit gpiointerger_driver_exit(void)
{
...
module_init(gpiointerger_driver_init);
module_exit(gpiointerger_driver_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("LongTD");
MODULE_DESCRIPTION("Interger-Based GPIO Driver for Raspberry Pi Zero 2w");  
MODULE_VERSION("1.0");
